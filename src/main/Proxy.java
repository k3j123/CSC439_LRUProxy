...
public class Proxy
{
    ...
    public Proxy(String inDirectory, int maxCacheSize, int sleepSeconds, int replacementMethod)
    {
    	// Set minimum sleep seconds to 0
        if (sleepSeconds<0)
        {
            sleepSeconds=0;
        }
        ...
        // Set minimum cache size to 1
        if (maxCacheSize<1)
        {
            maxCacheSize=1;
        }
        
        /** Set replacement method to be used:
         *  0 - Least Recently Used (LRU) (default)
         *  1 - Most Recently Used (MRU)
         *  2 - Least Frequently Used (LFU)
         *  3 - Random Replacement (RR)
         */
        if (replacementMethod >= 0 && replacementMethod <= 3)
        {
        	this.replacementMethod = replacementMethod;
        }
        else 
        {
        	this.replacementMethod = 0;
        }
    ...
    public void run()
    {
        ...
        do
        {
            ...
            if (url.trim().length()>0)
            {
                ...
                String removedURL=cacheList.addNewObject(url, hit, replacementMethod);
    ...
    public static void main(String args[])
    {
        if (args.length==4)
        {
            try
            {
                ...
                temp = args[3];
                int replacementMethod=Integer.parseInt(temp);		
                Proxy proxy=new Proxy(directory, maxCacheSize, sleepSeconds, replacementMethod);
                proxy.run();
            }
            catch (Exception e)
            {
        ...
        else
        {
            ...
            System.out.println("replacement method to be used: 0:LRU, 1:MRU, 2:LFU, 3:RR");
        }
    }
}
